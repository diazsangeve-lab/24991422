...
\midrule
Boys & 1-year & 0.950 & 0.981 & -0.031\\
Boys & 2-year & 0.891 & 0.958 & -0.067\\
Boys & 3-year & 0.836 & 0.934 & -0.098\\
Girls & 1-year & 0.937 & 0.952 & -0.014\\
Girls & 2-year & 0.868 & 0.900 & -0.032\\
\addlinespace
Girls & 3-year & 0.815 & 0.853 & -0.038\\
\bottomrule
\end{longtable}
\endgroup{}
...
A complementary view scales each standout name to its own peak, so the
shape and timing of each wave can be compared directly.
...
\tightlist
\item
  The Whitney effect is real and it generalises. Whitney rises with
  Houston in the mid 1980s, and the same pattern repeats for Sade,
  Aaliyah, Mya and Rihanna, each climbing in the years after the singer
  broke through.
\item
  The clearest cases are names the chart all but created. Sade and
  Rihanna barely existed before their first hit and then ran into the
  thousands.
\end{itemize}
...
Jeremy & Chitty Chitty Bang Bang & 1968 & 6.8 & 7495 & 28.9\\
...
\tightlist
\item
  Characters travel from the screen into the register. Dawson climbs
  after Dawson's Creek, Meadow after The Sopranos, and Whitley after A
  Different World, each within a year or two of the premiere.
\item
...
cultural moment landed gives an event study, the average naming response
to a hit or a premiere.
...
\tightlist
\item
  The response is fast. Both music and screen names lift within a year
  of the event and peak inside three, so naming tracks culture almost in
  real time.
\item
  Music moves harder than screen. A hit record lifts its name more
  sharply than a premiere does, which fits a song's instant, repeated
  national reach against a show's slower build.
\end{itemize}
